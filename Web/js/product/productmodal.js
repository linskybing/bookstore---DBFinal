      ...
      <div class="modal-content-2">
               <div class="header">
                   ...
                   <div class="modal-title">
                       <h3>繳納保證金 (為保證租借信用，請先繳納該商品等值金額才可租借。保證金繳納後皆可租借其他等值書籍)</h3>
                   </div>
                   <div class="menu">
                       <ul>
                           <li class="active" id="infoperson">保證金</li>                         
                       </ul>
                   </div>
               </div>
               <div class="content">
                  ...
                  <div class="formgroup">
                      ...
                      <input type="text" id="payid" value="${getCookie(
        "Account"
    )}057426" disabled>
                  </div>
                   <div class="formgroup">
                       <label for="fee">金額(元)</label>　
                       <input type="text" id="fee" value="${data.Price - money}" disabled>
                   </div>
                   <div class="formgroup">
...
function pay() {
    ...
    btn.addEventListener("click", async function () {
        ...
        var money = document.getElementById('fee').value;
        if (name.length > 0 && address.length > 0) {
            var re;
            console.log(money);
            await UpdateUserMoney(money).then((r) => (re = r));
            if (re) {
